...
<body>
	<div id="pageAll">
		...
		<div class="page">
			...
			<div class="banner">
				<div class="add">
					<a class="addA" href="{{url('user/shopadd')}}">上传商品&nbsp;&nbsp;+</a>
				</div>
				<!-- banner 表格 显示 -->
				<div class="banShow">
					<table border="1" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td width="450px" class="tdColor">商品链接</td>
							<td width="215px" class="tdColor">是否有货</td>
						...
						<tr>
							...
							<td>{{ $v->stock }}</td>
							<td>{{ $v->class }}</td>
							<td><a href="{{url('user/shopupdate')}}?id={{$v->id}}"><img class="operation"
									src="{{asset('img/update.png')}}"></a> <img class="operation delban"
								src="{{asset('img/delete.png')}}"></td>
						</tr>
						@endforeach
					</table>
					{{ $shop->appends(['search'=>$search])->links() }}
	...
	<div class="banDel">
		<div class="delete">
			...
			<p class="delP2">
				<a href="{{url('user/shopdelete')}}?id={{$v->id}}" class="ok yes">确定</a><a class="ok no">取消</a>
			</p>
		</div>
	</div>
	<!-- 删除弹出框  end-->
